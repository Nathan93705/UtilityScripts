    ...
    */
    export function register(data: CommandData, callBack: (player: Player, args: string[]) => void) {
        ...
        if (data.description.length < 1) {
            throw error(`Error Registerying Command; Command Description Must Be At Least 1 Character Long`)
        }
        if (data.name.length < 1) {
            throw error(`Error Registerying Command; Command Name Must Be At Least 1 Character Long`)
        }
